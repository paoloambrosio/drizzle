...
class TimedActionFactorySpec extends FlatSpec with Matchers with ScalaFutures {
  ...
  trait TestContext extends TimedActionFactory {
    ...
    implicit val t1clock: Clock = Clock.offset(t0clock, Duration.ofSeconds(2))
